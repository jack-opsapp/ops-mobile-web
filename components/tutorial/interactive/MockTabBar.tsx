...
export function MockTabBar({ activeTab, dimmed = false }: MockTabBarProps) {
  const tabs = [
  ...
  return (
    <div
      className={`flex items-center justify-around py-2 border-t border-white/10 transition-opacity duration-300 ${dimmed ? 'opacity-30' : ''}`}
      style={{ background: '#0A0A0A' }}
    >
      {tabs.map((tab) => {
        const isActive = tab.id === activeTab
        return (
          <div key={tab.id} className="flex flex-col items-center gap-0.5 px-4 py-1">
            <tab.icon active={isActive} />
            <span
              className={`font-kosugi text-[10px] ${
                isActive ? 'text-ops-accent' : 'text-ops-text-tertiary'
              }`}
            >
              {tab.label}
            </span>
...
function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" className={active ? 'text-ops-accent' : 'text-ops-text-tertiary'}>
